...
```sh
cargo build -p mock-runtime --release
```
...
The `minimal-template-node` has dependency on the `mock-runtime`. It will use
the `mock_runtime::WASM_BINARY` constant (which holds the WASM blob as a byte
